...
func CRSRules() []core.Rule {
	return []core.Rule{
		...
		{ID: "CRS-920230", Name: "CRS Multiple URL Encoding", Phase: core.PhaseRequestHeaders, Paranoia: 2, Category: "crs.protocol", Score: 30, Action: core.ActionBlock, Description: "CRS: multiple URL encodings detected", Targets: []string{"uri", "uri_raw"}, Pattern: `%25(?:[0-9a-fA-F]{2}|u[0-9a-fA-F]{4})`},
		{ID: "CRS-920240", Name: "CRS URL Encoding Overlong", Phase: core.PhaseRequestHeaders, Paranoia: 2, Category: "crs.protocol", Score: 40, Action: core.ActionBlock, Description: "CRS: UTF-8 overlong encoding", Targets: []string{"uri", "uri_raw"}, Pattern: `%[cC][01]%[89abAB][0-9a-fA-F]|%[eE]0%[89abAB][0-9a-fA-F]`},
		{ID: "CRS-920260", Name: "CRS Unicode Abuse", Phase: core.PhaseRequestHeaders, Paranoia: 2, Category: "crs.protocol", Score: 40, Action: core.ActionBlock, Description: "CRS: half-width / full-width Unicode abuse", Targets: []string{"uri", "args", "uri_raw"}, Pattern: `%u[fF][fF][0-9a-fA-F]{2}`},
		...
		// ============================================================
		{ID: "CRS-930100", Name: "CRS Path Traversal Unicode", Phase: core.PhaseRequestHeaders, Paranoia: 1, Category: "crs.lfi", Score: 80, Action: core.ActionBlock, Description: "CRS: Unicode/encoded path traversal", Targets: []string{"uri", "args", "body", "uri_raw"}, Pattern: `(?i)(?:%2e|%c0%ae|%c1%9c|\\u002e){2}[/\\]|(?:%2e){2}[/\\]|\.{2}(?:/|%2f|%5c|\\\\)`},
		...
		{ID: "CRS-942170", Name: "CRS SQLi Column Enum", Phase: core.PhaseRequestBody, Paranoia: 2, Category: "crs.sqli", Score: 50, Action: core.ActionBlock, Description: "CRS: UNION SELECT column enumeration", Targets: []string{"args", "body"}, Pattern: `(?i)\bunion\b(?:\s+(?:all|distinct))?\s+select\b[\s\S]{0,200}\b(?:from|where|limit)\b`},
		...
		// headers target dropped — `H4sIA` is the base64 prefix of gzip data
		// (gzip magic 1f8b08 -> H4sIA), which is routine in compressed trace /
		// telemetry headers (X-Cloud-Trace, OpenTelemetry baggage) and cookies;
		// `rO0`/`aced0005` likewise collide with base64/hex tokens. The actual
		// serialized payload arrives in args/body, still scanned here.
		{ID: "CRS-944110", Name: "CRS Java Serialization", Phase: core.PhaseRequestBody, Paranoia: 1, Category: "crs.java", Score: 80, Action: core.ActionBlock, Description: "CRS: Java serialized object marker", Targets: []string{"args", "body"}, Pattern: `(?:aced0005|rO0[A-Z]|H4sIA)`},
