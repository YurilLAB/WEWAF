...
func DefaultRules() []core.Rule {
	return []core.Rule{
		...
		{ID: "XSS-002", Name: "XSS JavaScript Protocol", Phase: core.PhaseRequestHeaders, Score: 100, Action: core.ActionBlock, Description: "javascript: protocol in URL/header", Targets: []string{"args", "headers"}, Pattern: `(?i)java(?:\s|%0[0-9a-fA-F]|%1[0-9a-fA-F]|%7[fF]|%20|&#[xX]?[0-9a-fA-F]+;)*script(?:\s|%0[0-9a-fA-F]|%1[0-9a-fA-F]|%7[fF]|%20|&#[xX]?[0-9a-fA-F]+;)*:`},
		...
		{ID: "SQLI-010", Name: "JSON SQL Injection", Phase: core.PhaseRequestBody, Score: 70, Action: core.ActionBlock, Description: "SQL injection payload inside JSON string value", Targets: []string{"args", "body", "headers"}, Pattern: `(?i)"[^"]*(?:union\s+(?:(?:all|distinct)\s+)?select|'\s*or\s+'1'\s*=\s*'1|"\s*or\s+"1"\s*=\s*"1|;\s*(?:drop|delete|truncate|insert|update)\s|(?:and|or)\s+\d+\s*=\s*\d+)[^"]*"`},
		...
		{ID: "SQLI-024", Name: "Comment-based evasion", Phase: core.PhaseRequestHeaders, Score: 60, Action: core.ActionBlock, Description: "MySQL /*!...*/ or -- evasion", Targets: []string{"args", "body"}, Pattern: `(?i)/\*!\d{5}.*\*/|\bunion\b\s*(?:/\*.*?\*/\s*)?(?:(?:all|distinct)\s+(?:/\*.*?\*/\s*)?)?select`},
